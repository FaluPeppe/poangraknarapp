...
function rendera(historik, resultat, on401) {
  ...
  // ---- Tidsperiod ----
    ...
    [
      ...
      radio.onchange = () => { vald_period = p.varde; rendera(historik, resultat, on401); };
    ...
    [
      ...
      radio.onchange = () => { vald_visa = v.varde; };
      radRad.appendChild(radio);
      radRad.appendChild(document.createTextNode(" " + v.etikett));
      periodForm.appendChild(radRad);
    });
    
    const visaKnapp = document.createElement("button");
    visaKnapp.className = "knapp-slumpa";
    visaKnapp.textContent = "Visa statistik";
    visaKnapp.onclick = () => hamtaStatistik(historik, on401);
    periodForm.appendChild(visaKnapp);
